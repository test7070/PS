...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr">
	<head>
		...
		<script type="text/javascript">
			...
			var q_readonlys = ['txtMoney','txtSprice','txtOrdeno','txtNo2'];
			var bbmNum = [];
			var bbsNum;
			var bbmMask = [];
			var bbsMask = [];
			...
			function sum() {
				...
				for (var j = 0; j < q_bbsCount; j++) {
					...
					t_counta = (q_float('txtDiscount_' + j) ? q_div(q_float('txtDiscount_' + j),100): 1);
					if($('#chkAprice_'+j).prop('checked')){
                        t_moneys = q_float('txtTotal_' + j);
                        t_moneyuss = q_float('txtTotalus_' + j);
                    }
                    else{
                        if (t_unit.length == 0 || t_unit == 'KG' || t_unit == 'M2' || t_unit == 'M²' || t_unit == 'M' || t_unit == '批' || t_unit == '公斤') {
                            t_moneys = q_mul(q_mul(q_mul(t_prices, t_weights),t_counta),t_float==0?1:t_float);
                            if(t_float!=0)
                                 t_moneyuss = q_mul(q_mul(t_prices, t_weights),t_counta);
                        } else {
                            t_moneys = q_mul(q_mul(q_mul(t_prices, t_mounts),t_counta),t_float==0?1:t_float);
                            if(t_float!=0)
                                t_moneyuss = q_mul(q_mul(t_prices, t_mounts),t_counta);
                        }
					...
					case 'ordc_import':
                        if (b_ret != null) {
                        	...
                        	var curItem,newArray= new Array();
                        		...
                        		q_gridAddRow(bbsHtm, 'tbbs', 'txtProductno,txtProduct,txtSpec,combSpec,txtDime,txtWidth,txtLengthb,txtRadius,txtLengthc,txtSize,txtOrdeno,txtNo2,txtPrice,txtMount,txtWeight,txtTotal,txtMemo,txtUnit,txtSource,txtStyle,txtClass'
	                        	, newArray.length, newArray
								, 'productno,product,spec,spec,dime,width,lengthb,radius,lengthc,size,noa,no2,price,emount,eweight,total,memo,unit,source,style,class', 'txtProductno');   	
                        	sum();
                        }else{
                        	Unlock(1);
                        }
		...
		<div class='dbbs'>
			<table id="tbbs" class='tbbs' style=' text-align:center'>
				<tr style='color:white; background:#003366;' >
					...
					<td align="center" style="width:50px;"><a id='lblStyle_st'> </a></td>
					<td align="center" style="width:50px;" class="RK_hide"><a>等級</a></td>
					<td align="center" style="width:100px;"><a>鋼廠</a><BR><a id='lblSize_ps'>板面</a></td>
					<td align="center" style="width:380px;" id='Size'>
                        <a id='lblSize_help'>厚度 x 寬度/OD x 長度/ID x 寬度2 x 寬度3 </a>
                    </td>
					
					<td align="center" style="width:80px;"><a id='lblMount_st'> </a></td>
					<td align="center" style="width:80px;"><a id='lblWeights_st2'>重量<BR>毛重</a></td>
				...
				<tr style='background:#cad3ff;'>
					...
					<td><input id="txtSource.*" type="text" style="width:95%;"/>
					...
					<td>
						...
						<div id="x4.*" style="float: left;display:block;width:20px;padding-top: 4px;">x</div>
                        <input class="txt num" id="textSize5.*" type="text"  style="float: left;width:55px;" disabled="disabled"/>
						<!--上為虛擬下為實際-->
					...
					<td>
                        <input id="txtDiscount.*" type="text" class="txt num" style="width:95%;"/>
                    </td>
					...
					<td>
                        <input id="txtBdime.*" type="text" class="txt num" style="width:95%;"/>
                        <input id="txtBwidth.*" type="text" class="txt num" style="width:95%;"/>
                    </td>
					<td class="Ordctds">
